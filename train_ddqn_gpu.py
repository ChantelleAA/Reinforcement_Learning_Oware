...
#     plt.plot(np.cumsum(rewards_random), "b", label="Random agent")
...
#     plt.subplot(2, 3, 5)
#     plt.plot(rewards_learner, "k--", label="DQN Agent Rewards")
#     plt.plot(rewards_random, "p--", label="Random Agent Rewads")
#     plt.xlabel("Time Steps")
#     plt.ylabel("Reward")
#     plt.legend()   

#     plt.subplot(2, 3, 6)
#     plt.plot(loss_list, "m", label="DQN Agent")
#     plt.xlabel("Loss")
#     plt.ylabel("Length")
#     plt.legend() 
...
# train_agents(5, game_controller, './saved_weights_ddqn/E{}.weights.h5', './saved_weights_ddqn/model_params{}.json', agent_type='ddqn')
...
def train_agents(episodes, game_controller, save_weights_path, save_model_path, agent_type='ddqn'):
    ...
    with open('training_data.csv', mode='w', newline='') as file:
        ...
        writer.writerow(["Episode", "Loss", "Average Reward", "Winner", "Epsilon"])
        for i in range(episodes):
            writer.writerow([i+1, training_losses[i] if i < len(training_losses) else 'N/A', 
                             average_rewards_per_episode[i], winners[i], epsilon_over_episodes[i]])
